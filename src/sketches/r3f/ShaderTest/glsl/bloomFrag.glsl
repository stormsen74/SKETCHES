...
void main() {
    ...
    vec2 texel = vUv;
    vec4 pixel = vec4(texture2D(u_map, texel));
    ...
    if (pixel.a < 1.0) {
        ...
        bloom += (texture2D(u_map, vec2(texel.x, texel.y)) - dim);
        bloom += (texture2D(u_map, vec2(texel.x - glow, texel.y - glow)) - dim);
        bloom += (texture2D(u_map, vec2(texel.x + glow, texel.y + glow)) - dim);
        bloom += (texture2D(u_map, vec2(texel.x + glow, texel.y - glow)) - dim);
        bloom += (texture2D(u_map, vec2(texel.x - glow, texel.y + glow)) - dim);
        bloom += (texture2D(u_map, vec2(texel.x + glow, texel.y)) - dim);
        bloom += (texture2D(u_map, vec2(texel.x - glow, texel.y)) - dim);
        bloom += (texture2D(u_map, vec2(texel.x, texel.y + glow)) - dim);
        bloom += (texture2D(u_map, vec2(texel.x, texel.y - glow)) - dim);
